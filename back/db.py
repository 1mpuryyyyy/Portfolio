...
class Database:
    ...
    def crate_recorts_reg(self, login: str, email: str, password: str):
        f = self.con('SELECT login FROM peoples_data_reg ORDER BY id DESC LIMIT 1;', fetchall=True)
        f = 0 if f == [] else f[0][0]
    ...
    def get_values(self, login):
        try:
            values = self.con('SELECT * FROM peoples_data_reg WHERE login=?', params=(login,), off=True,
                              fetchall=True)
            return values
